package com.corporacao.capsula.componentes.calendario.widget;
...
import com.corporacao.capsula.componentes.calendario.data.CalendarAdapter;
import com.corporacao.capsula.componentes.calendario.data.Day;
import com.corporacao.capsula.componentes.calendario.data.Event;
import com.corporacao.capsula.componentes.calendario.view.ExpandIconView;
    ...
    protected void redraw() {
        ...
        if (mAdapter != null) {
            for (int i = 0; i < mAdapter.getCount(); i++) {
                ...
                ImageView lImageView = view.findViewById(R.id.img_event_tag);
...
//                for (Event lEvent: mEventList) {
//                    if (day.getYear() == lEvent.getYear()
//                            && day.getMonth() == lEvent.getMonth()
//                            && day.getDay() == lEvent.getDay()) {
//                        lImageView.setVisibility(View.VISIBLE);
//                        lImageView.setColorFilter(lImageView.getColor(), PorterDuff.Mode.SRC_ATOP);
//                    }
//                }
            }
        }
    ...
    public void clearEventList() {
        mAdapter.clearEventList();
        reload();
    }

    public void addEventTag(int numYear, int numMonth, int numDay) {
        Event lEvent = new Event(numYear, numMonth, numDay ,getEventColor());
        mEventList.add(lEvent);
        mAdapter.addEvent(lEvent);
        reload();
    }

    public void addEventTag(int numYear, int numMonth, int numDay, int color) {
        Event lEvent = new Event(numYear, numMonth, numDay ,color);
        mEventList.add(lEvent);
        mAdapter.addEvent(lEvent);
        reload();
    }
